...
class RunAco:
    def __init__(self, problem_name, file_path, num_ants, evaporation_rate, Q, num_nodes, num_iter):
        """
        ...
        self.run(file_path, num_ants, evaporation_rate, Q, num_nodes, num_iter)

    # create functions to read in the files and get the parameters needed to run ACO.
    def run(self, file_path, num_ants, evaporation_rate, Q, num_nodes, num_iter):
        weight_matrix = np.loadtxt(file_path)
        adj_matrix = np.ones(weight_matrix.shape)  # only works for fully connected graphs
        for i in range(len(adj_matrix)):
            adj_matrix[i][i] = 0.0
        aco = AntColonyOptimization(num_ants, evaporation_rate, Q, num_nodes, adj_matrix, weight_matrix, num_iter)
        aco.run()
...
def main():
    ...
    num_nodes = 48
    num_iter = 1000
    runAco = RunAco(problem_name, file_path, num_ants, evaporation_rate, Q, num_nodes, num_iter)
